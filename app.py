...
try:
    ...
    if selected_tab == "Мінування":
        try:
            ...
            # Додаємо українські назви місяців
            df_m['Month_Num'] = df_m['Дата_dt'].dt.month
            df_m['Year'] = df_m['Дата_dt'].dt.year
            df_m['Місяць_Укр'] = df_m['Month_Num'].map(MONTHS_UKR) + " " + df_m['Year'].astype(str)
            df_m['Sort_Key'] = df_m['Year'].astype(str) + df_m['Month_Num'].astype(str).str.zfill(2)

            # --- ГРАФІК 1: ДЕТАЛЬНО ЗА МІСЯЦЬ ---
            st.markdown("### 📅 Щоденна інтенсивність")
            
            # Сортуємо доступні місяці для вибору (новіші зверху)
            month_options = df_m[['Місяць_Укр', 'Sort_Key']].drop_duplicates().sort_values('Sort_Key', ascending=False)
            sel_month_label = st.selectbox("Оберіть місяць:", month_options['Місяць_Укр'])
            
            monthly_data = df_m[df_m['Місяць_Укр'] == sel_month_label]
            # Групуємо по датах (строках) для відображення на осі Х
            daily_sum = monthly_data.groupby(monthly_data['Дата_dt'].dt.strftime('%d.%m'))['Кількість'].sum().reset_index()

            fig1 = go.Figure()
            fig1.add_trace(go.Bar(
                x=daily_sum.iloc[:, 0], y=daily_sum['Кількість'],
            ...
            # --- ГРАФІК 2: ЗАГАЛЬНИЙ ПІДСУМОК ---
            st.write("---")
            st.markdown("### 📈 Загальний підсумок по місяцях")
            
            overall_monthly = df_m.groupby(['Sort_Key', 'Місяць_Укр'])['Кількість'].sum().reset_index()
            overall_monthly = overall_monthly.sort_values('Sort_Key')

            fig2 = go.Figure()
            fig2.add_trace(go.Bar(
                x=overall_monthly['Місяць_Укр'], 
            ...
            with st.expander("📂 АРХІВ ЗАПИСІВ"):
                st.dataframe(df_m[['Дата', 'Об\'єкт', 'Кількість', 'Район']], use_container_width=True, hide_index=True)

        except Exception as e:
            st.error(f"Помилка даних мінування: {e}")
