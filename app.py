...
def set_design(bin_file):
    ...
    st.markdown(f"""
        ...
        table {{ background-color: rgba(255,255,255,0.05) !important; color: white !important; border-radius: 10px; }}
        thead tr th {{ background-color: rgba(0,0,0,0.3) !important; color: #ffd700 !important; }}
...
def calculate_verif_data(qty, target, status):
    """Повертає (Верифіковані бали, Верифікована кількість)"""
    ...
    if category == "⚔️ Бригадні звіти":
        ...
        if clean_results:
            ...
            with tab_cum:
                ...
                fig_cum.update_layout(barmode='group', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="white", height=500, legend=dict(orientation="h", y=1.1))
            ...
            with tab_daily:
                ...
                fig_daily.update_layout(barmode='group', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="white", height=500, legend=dict(orientation="h", y=1.1))
        ...
        for r in data_list:
            ...
            if pd.notnull(dt):
                ...
                if "не верифіковано" in st_cl:
                    ...
                    unv = float(m.group(1)) if m else qty
                    v_q = max(0, qty - unv); u_q = unv
                else: v_q = qty; u_q = 0
    ...
    # --- КАТЕГОРІЯ: УРАЖЕННЯ ---
        ...
        for r in data_list:
            ...
            if last_dt and str(r[1]).strip() != "":
                q = to_native(r[2])
                v_pts, v_qty = calculate_verif_data(q, r[1], r[3])
                clean_rows.append({"Дата": last_dt, "V": v_pts, "U": (q * POINTS_MAP.get(str(r[1]).strip(), 0)) - v_pts, "Ціль": str(r[1]).strip(), "Q": q, "QV": v_qty})
        if clean_rows:
