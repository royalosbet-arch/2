...
# =================================================================
# 2. ДОПОМІЖНІ ФУНКЦІЇ ТА СТИЛІЗАЦІЯ
# =================================================================
def get_base64(path):
    try:
...
def style_tactical_table(df):
    """Ховає випадкові індекси та розфарбовує верифікацію цілей"""
    styler = df.style.hide()  # ПОВНІСТЮ ХОВАЄ СТОВПЧИК З "РАНДОМНИМИ" ЦИФРАМИ
    
    # Стилізація кольорів (якщо значення > 0 — підсвічуємо яскраво, якщо 0 — робимо напівпрозорим)
    if "Верифіковано (шт)" in df.columns:
        styler = styler.map(lambda v: "color: #2ECC71; font-weight: 800;" if v > 0 else "color: rgba(46, 204, 113, 0.2);", subset=["Верифіковано (шт)"])
    if "Не verif (шт)" in df.columns:
        styler = styler.map(lambda v: "color: #E74C3C; font-weight: 800;" if v > 0 else "color: rgba(231, 76, 60, 0.2);", subset=["Не verif (шт)"])
    if "На верифікації (шт)" in df.columns:
        styler = styler.map(lambda v: "color: #95A5A6; font-weight: 800;" if v > 0 else "color: rgba(149, 165, 166, 0.2);", subset=["На верифікації (шт)"])
...
st.markdown(f"""
    ...
    table {{ background-color: rgba(255,255,255,0.05) !important; color: white; border-radius: 10px; width: 100%; }}
    ...
    tbody tr td {{ background-color: transparent !important; border-bottom: 1px solid rgba(255,255,255,0.05) !important; }}
    ...
    if category == "⚔️ Бригадні звіти":
                ...
                if b_name == "1аемб":
                    ...
                    for r in u_rows[1:]:
                        ...
                        if l_dt and str(r[1]).strip() not in ["", "-", "•", ".", "Ціль"]:
                            ...
                            vp, vq = get_urazh_data(qty, str(r[1]), str(r[3]))
                            
                            # Розподіл статусів верифікації
                            q_ver = vq
                            if q_ver > 0: q_unver, q_pend = 0.0, 0.0
                            elif "не верифік" in st_clean: q_unver, q_pend = qty, 0.0
                            else: q_unver, q_pend = 0.0, qty
                                
                            all_results.append({"D": l_dt, "B": b_name, "T": str(r[1]), "PU": vp, "PM": 0.0, "QT": qty, "QV": q_ver, "QUN": q_unver, "QPE": q_pend})
                    ...
                    for r in m_rows[1:]:
                        ...
                        if pd.notnull(dt_m):
                            ...
                            vq_m, unv_m = get_mine_data(qty_m, str(r[3]))
                            
                            if "не верифік" in st_cl: qp_m = 0.0
                            elif st_cl == "верифіковано": unv_m, qp_m = 0.0, 0.0
                            else: vq_m, unv_m, qp_m = 0.0, 0.0, qty_m
                                
                            if qty_m > 0:
                                all_results.append({"D": dt_m, "B": b_name, "T": "Мінування", "PU": 0.0, "PM": vq_m, "QT": qty_m, "QV": vq_m, "QUN": unv_m, "QPE": qp_m})
                else:
                    ...
                    for r in u_rows[1:]:
                        ...
                        if target not in ["", "-", "•", ".", "Ціль"]:
                            ...
                            if vq > 0: q_unver, q_pend = 0.0, 0.0
                            elif "не верифік" in st_clean: q_unver, q_pend = qty, 0.0
                            else: q_unver, q_pend = 0.0, qty
                                
                            all_results.append({"D": l_dt, "B": b_name, "T": target, "PU": vp, "PM": 0.0, "QT": qty, "QV": vq, "QUN": q_unver, "QPE": q_pend})
                        if len(r) > 4:
                            v_mine, _ = get_mine_data(to_native(r[4]), "Верифіковано")
                            if v_mine > 0:
                                all_results.append({"D": l_dt, "B": b_name, "T": "Мінування", "PU": 0.0, "PM": v_mine, "QT": v_mine, "QV": v_mine, "QUN": 0.0, "QPE": 0.0})
            except: continue
        ...
        if all_results:
            ...
            for t in sorted(list(set([r["T"] for r in u_res]))):
                u_table.append({
                    ...
                    "Не verif (шт)": int(sum(r.get("QUN", 0) for r in u_res if r["T"] == t)),
                    "На верифікації (шт)": int(sum(r.get("QPE", 0) for r in u_res if r["T"] == t)),
            ...
            def draw_chart(mode):
                ...
                fig.update_layout(barmode='group', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="white", height=500, xaxis=dict(type='category', tickangle=-45), margin=dict(t=80))
                return fig
        ...
        for r_u in raw_u:
            ...
            if last_dt and str(r_u[1]).strip() not in ["", "-", "•", ".", "Ціль"]:
                ...
                vp, vq = get_urazh_data(qty, str(r_u[1]), str(r_u[3]))
                
                if vq > 0: q_ver, q_unver, q_pend = qty, 0.0, 0.0
                elif "не верифік" in st_clean: q_ver, q_unver, q_pend = 0.0, qty, 0.0
                else: q_ver, q_unver, q_pend = 0.0, 0.0, qty
                
                clean_u.append({
                    "D": last_dt, "V": vp, "U": (qty * POINTS_MAP.get(str(r_u[1]).strip(), 0)) - vp, 
                    "T": str(r_u[1]), "QT": qty, "QV": q_ver, "QUN": q_unver, "QPE": q_pend
                })
        if clean_u:
            ...
            for r_u in clean_u:
                ...
                n = r_u["T"]
                if n not in obs: obs[n] = {"QT": 0, "QV": 0, "QUN": 0, "QPE": 0, "V": 0}
                obs[n]["QT"] += r_u["QT"]
                obs[n]["QV"] += r_u["QV"]
                obs[n]["QUN"] += r_u["QUN"]
                obs[n]["QPE"] += r_u["QPE"]
