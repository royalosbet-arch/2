            ...
            try:
                ...
                for r in u_rows[1:]:
                    ...
                    if target not in ["", "-", "•", ".", "Ціль"]:
                        ...
                        # А. ЛОГІКА: Мінування записано як звичайний рядок в таблиці
                        if target == "Мінування":
                    ...
                    # В. ЗБЕРЕЖЕННЯ ЗВОРОТНЬОЇ СУМІСНОСТІ (Старий формат із 5-ї колонки Excel)
        ...
        for b_name in unit_names:
            ...
            try:
                ...
                for r in u_rows[1:]:
                    ...
                    # Фільтр по місяцю та року одразу
                    ...
                    if target == "Мінування":
                        # ЛОГІКА ПІДРАХУНКУ МІНУВАННЯ (така ж, як у бригадних звітах)
                        if "не вериф" in st_clean:
                            match = re.search(r'(\d+)', st_clean)
                            qun_m = float(match.group(1)) if match else qty
                            qv_m = max(0.0, qty - qun_m)
                        ...
                        # Додаткова перевірка 5-ї колонки для зворотної сумісності
                        if len(r) > 4:
                            v_mine = to_native(r[4])
                            if v_mine > 0:
                                qv_m += v_mine 
...
# =================================================================
# 5. БЛОК ТЕХНІЧНОЇ ПІДТРИМКИ (КОМПАКТНИЙ, В ЛІВОМУ НИЖНЬОМУ КУТКУ)
# =================================================================
st.markdown("""
    <style>
    .support-card {
