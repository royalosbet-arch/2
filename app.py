...
# =================================================================
# 2. ДОПОМІЖНІ ФУНКЦІЇ (ОНОВЛЕНІ ПОДІЛИ СТАТУСІВ)
# =================================================================
def get_base64(path):
    try:
...
def get_urazh_data(qty, target, status):
    """Повертає: (Бали, Верифіковано_шт, Не_верифіковано_шт, На_верифікації_шт)"""
    t_clean = str(target).strip()
    unit_p = POINTS_MAP.get(t_clean, 0)
    st_clean = str(status).lower().strip()
    
    if st_clean == "верифіковано":
        return qty * unit_p, qty, 0.0, 0.0
    elif "не верифіковано" in st_clean or st_clean == "мінус" or st_clean == "списано":
        return 0.0, 0.0, qty, 0.0
    else:
        # Сюди потрапляють статуси "в процесі", "на верифікації", "очікує" або пусті значення
        return 0.0, 0.0, 0.0, qty

def get_mine_data(qty, status):
    """Повертає: (Верифіковано_шт, Не_верифіковано_шт, На_верифікації_шт)"""
    ...
    if "не верифіковано" in st_cl:
        ...
        return max(0.0, qty - unv_q), unv_q, 0.0
    elif st_cl == "верифіковано":
        return qty, 0.0, 0.0
    elif "процес" in st_cl or "очікує" in st_cl or st_cl == "на верифікації":
        return 0.0, 0.0, qty
    return 0.0, 0.0, qty
...
st.markdown(f"""
    ...
    table {{ background-color: rgba(0,0,0,0.5) !important; color: white !important; border-radius: 10px; width: 100%; }}
    thead tr th {{ background-color: rgba(0,0,0,0.75) !important; color: #ffd700 !important; font-weight: 800 !important; border-bottom: 1px solid rgba(255,215,0,0.2) !important; }}
...
try:
    if category == "⚔️ Бригадні звіти":
                ...
                if b_name == "1аемб":
                    ...
                    for r in u_rows[1:]:
                        ...
                        if l_dt and str(r[1]).strip() not in ["", "-", "•", ".", "Ціль"]:
                            vp, vq, vnq, vqp = get_urazh_data(to_native(r[2]), str(r[1]), str(r[3]))
                            all_results.append({
                                "D": l_dt, "B": b_name, "T": str(r[1]), "PU": vp, "PM": 0.0, 
                                "QT": to_native(r[2]), "QV": vq, "QNV": vnq, "QNP": vqp
                            })
                    ...
                    for r in m_rows[1:]:
                        ...
                        if pd.notnull(dt_m):
                            vq_m, vnq_m, vqp_m = get_mine_data(to_native(r[2]), str(r[3]))
                            if (vq_m + vnq_m + vqp_m) > 0:
                                all_results.append({
                                    "D": dt_m, "B": b_name, "T": "Мінування", "PU": 0.0, "PM": vq_m, 
                                    "QT": to_native(r[2]), "QV": vq_m, "QNV": vnq_m, "QNP": vqp_m
                                })
                else:
                    ...
                    for r in u_rows[1:]:
                        ...
                        if target not in ["", "-", "•", ".", "Ціль"]:
                            vp, vq, vnq, vqp = get_urazh_data(to_native(r[2]), target, str(r[3]))
                            all_results.append({
                                "D": l_dt, "B": b_name, "T": target, "PU": vp, "PM": 0.0, 
                                "QT": to_native(r[2]), "QV": vq, "QNV": vnq, "QNP": vqp
                            })
                        if len(r) > 4:
                            v_mine, vn_mine, vp_mine = get_mine_data(to_native(r[4]), "Верифіковано")
                            if (v_mine + vn_mine + vp_mine) > 0:
                                all_results.append({
                                    "D": l_dt, "B": b_name, "T": "Мінування", "PU": 0.0, "PM": v_mine, 
                                    "QT": to_native(r[4]), "QV": v_mine, "QNV": vn_mine, "QNP": vp_mine
                                })
            except: continue
        ...
        if all_results:
            ...
            for t in sorted(list(set([r["T"] for r in u_res]))):
                u_table.append({
                    ...
                    "Не верифіковано (шт)": int(sum(r["QNV"] for r in u_res if r["T"] == t)),
                    "На верифікації (шт)": int(sum(r["QNP"] for r in u_res if r["T"] == t)),
            ...
            st.markdown("---")
            st.metric("БРИГАДА (ВЕРИФІКОВАНО ЗА ТРАВЕНЬ):", int(sum(r["PU"] + r["PM"] for r in filtered)))
        ...
        for r in raw_m:
            dt = pd.to_datetime(str(r[0]), dayfirst=True, errors='coerce')
            if pd.notnull(dt):
                vq, uq, pq = get_mine_data(to_native(r[2]), r[3])
                m_list.append({"D": dt, "Month": MONTHS_UKR.get(dt.month, "") + " " + str(dt.year), "V": vq, "U": uq, "P": pq})
        if m_list:
            ...
            vv, uu, pp = {l:0.0 for l in labs}, {l:0.0 for l in labs}, {l:0.0 for l in labs}
            for r in m_data:
                l = f"{r['D'].day}.{str(r['D'].month).zfill(2)}"
                if l in vv: vv[l] += r["V"]; uu[l] += r["U"]; pp[l] += r["P"]
            ...
            fm.add_trace(go.Bar(x=labs, y=[pp[l] for l in labs], name='На верифікації', marker_color='#FFC000'))
            fm.add_trace(go.Scatter(x=labs, y=[vv[l]+uu[l]+pp[l] for l in labs], mode='text', text=[str(int(vv[l])) if vv[l]>0 else "" for l in labs], textposition='top center', showlegend=False, textfont=dict(color='white')))
        ...
        sel_ur = st.selectbox("ОБЕРІТЬ ПЕРІОД:", ["05.2026", "04.2026", "03.2026", "02.2026", "01.2026"])
        ...
        for r_u in raw_u:
            ...
            if last_dt and str(r_u[1]).strip() not in ["", "-", "•", ".", "Ціль"]:
                vp, vq, vnq, vqp = get_urazh_data(to_native(r_u[2]), str(r_u[1]), str(r_u[3]))
                clean_u.append({
                    "D": last_dt, "V": vp, "U": vnq * POINTS_MAP.get(str(r_u[1]).strip(), 0), 
                    "P": vqp * POINTS_MAP.get(str(r_u[1]).strip(), 0), "T": str(r_u[1]), 
                    "QT": to_native(r_u[2]), "QV": vq, "QNV": vnq, "QNP": vqp
                })
        if clean_u:
            ...
            for r_u in clean_u:
                ...
                if l in vv: vv[l] += r_u["V"]; uu[l] += r_u["U"]; pp[l] += r_u["P"]
                n = r_u["T"]
                if n not in obs: obs[n] = [0, 0, 0, 0, 0] # Всього, Вериф, Не вериф, На вер, Бали
                obs[n][0] += r_u["QT"]
                obs[n][1] += r_u["QV"]
                obs[n][2] += r_u["QNV"]
                obs[n][3] += r_u["QNP"]
            ...
            fu.add_trace(go.Bar(x=labs, y=[pp[l] for l in labs], name='На верифікації', marker_color='#FFC000'))
            fu.add_trace(go.Scatter(x=labs, y=[vv[l]+uu[l]+pp[l] for l in labs], mode='text', text=[str(int(vv[l])) if vv[l]>0 else "" for l in labs], textposition='top center', showlegend=False, textfont=dict(color='white')))
            ...
            st.table(pd.DataFrame([{
                "Тип цілі": k, "Всього": int(v[0]), "Верифіковано": int(v[1]), 
                "Не верифіковано": int(v[2]), "На верифікації": int(v[3]), "Бали": int(v[4])
            } for k, v in sorted(obs.items(), key=lambda x: x[1][4], reverse=True)]))
