...
import re
import time
import calendar # Додано для визначення кількості днів у місяці
...
try:
    if category == "⚔️ Бригадні звіти":
        ...
        # ВИПРАВЛЕНО: Яскраво-зелений колір (#00E676) для кращої видимості на темному фоні
        ...
        if all_results:
            ...
            # =================================================================
            # НОВИЙ БЛОК: ПРОГНОЗ ТА МЕТРИКИ (Варіант 1)
            # =================================================================
            now = datetime.now()
            current_day = now.day
            days_in_month = calendar.monthrange(cur_y, cur_m)[1]
            
            # Захист від ділення на нуль та виходу за межі місяця
            if current_day > days_in_month:
                current_day = days_in_month
            if current_day == 0:
                current_day = 1
                
            daily_avg = u_total_pts / current_day
