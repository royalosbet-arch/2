...
try:
    if category == "⚔️ Бригадні звіти":
        # Вибір періоду на початку сторінки звіту
        sel_report_month = st.selectbox("ОБЕРІТЬ МІСЯЦЬ ДЛЯ ПЕРЕГЛЯДУ ЗВІТУ:", ["05.2026", "04.2026"])
        prefix = sel_report_month.split(".")[0]  # Отримуємо "05" або "04"
        cur_m = int(prefix)
        ...
        for b_name in unit_names:
            # Формуємо динамічну назву листа (наприклад, 04.1аемб)
            sheet_name = f"{prefix}.{b_name}"
            
            try:
                # Пробуємо прочитати новий формат листа (наприклад: 05.1аемб)
                df_unit = conn.read(worksheet=sheet_name, ttl=300, header=None).fillna("")
            except:
                try:
                    # Якщо такого листа немає, пробуємо старий загальний формат листа
            ...
            # Окремо для 1аемб перевіряємо також загальний лист "Мінування", фільтруючи за датою
            if b_name == "1аемб":
                try:
