...
try:
    ...
    if selected_tab == "Мінування":
        # Робимо копію і ставимо заголовки
        df_m = raw_df.copy()
        df_m.columns = df_m.iloc[0]
        df_m = df_m.iloc[1:].reset_index(drop=True)
        
        # ОЧИСТКА ТА ПЕРЕТВОРЕННЯ ДАТ (максимально агресивно)
        ...
        if not df_m.empty:
            ...
            # Сортуємо список місяців (новіші зверху)
            ...
            fig1.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="white", height=400, xaxis_title="Число місяця")
            st.plotly_chart(fig1, use_container_width=True)

            # ГРАФІК 2: ЗАГАЛЬНИЙ ПІДСУМОК
            st.markdown("### 📈 Всього встановлено по місяцях")
            ...
            st.warning("У вкладці 'Мінування' не знайдено коректних дат. Перевірте формат в Excel (напр. 01.12.2025).")

    elif selected_tab == "Бригадний ЗГ":
        # Код для 3-х графіків бригади
