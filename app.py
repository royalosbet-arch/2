...
# =================================================================
# 2. ДОПОМІЖНІ ФУНКЦІЇ ТА ТАКТИЧНА СТИЛІЗАЦІЯ
# =================================================================
def get_base64(path):
    try:
...
def style_tactical_table(df):
    """Ховає технічні індекси та додає кольорову індикацію для верифікації цілей"""
    styler = df.style.hide()  # Приховуємо стовпчик з випадковими цифрами
    
    # Зелений колір для верифікованих цілей
    ...
    # Червоний колір для не верифікованих цілей
    if "Не верифіковано (шт)" in df.columns:
        styler = styler.map(lambda v: "color: #E74C3C; font-weight: 800;" if v > 0 else "color: rgba(231, 76, 60, 0.2);", subset=["Не верифіковано (шт)"])
    
    # Сірий колір для цілей на верифікації
                ...
                if b_name == "1аемб":
                    ...
                    for r in u_rows[1:]:
                        ...
                        if target not in ["", "-", "•", ".", "Ціль"]:
                            ...
                            all_results.append({"D": l_dt, "B": b_name, "T": target, "PU": vp, "PM": 0.0, "QT": qty, "QV": q_ver, "QUN": q_unver, "QPE": q_pend})
                        if len(r) > 4:
        ...
        if all_results:
            ...
            for t in sorted(list(set([r["T"] for r in u_res]))):
                u_table.append({
                    ...
                    "Не верифіковано (шт)": int(sum(r.get("QUN", 0) for r in u_res if r["T"] == t)),
