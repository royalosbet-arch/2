...
def check_password():
    if "password_correct" not in st.session_state:
        ...
        with col_c:
            st.markdown("<div style='padding:40px; background:rgba(255,255,255,0.05); border-radius:20px; border:1px solid rgba(255,255,255,0.1); text-align:center;'><h2 style='color:white;'>🛡️ СИТУАЦІЙНИЙ ЦЕНТР</h2><p style='color:#888;'>1 аемб. АВТОРИЗАЦІЯ</p></div>", unsafe_allow_html=True)
...
category = st.sidebar.radio("Напрямок:", ["🧨 Мінування", "🔥 Ураження", "📈 Рейтинг та Бали", "📡 Спец. розділи"])

selected_tab = ""
if category == "🧨 Мінування": selected_tab = "Мінування"
...
try:
    ...
    if selected_tab == "Е-Бали":
        # ВІДТВОРЕННЯ ГРАФІКА З ЕКСЕЛЯ
        try:
            # Припустимо, у вас у таблиці Е-Бали є колонки: 'Дата', 'Лютий', 'Березень'
            # Якщо назви інші — код спробує підлаштуватися
            x_axis = df.iloc[:, 0] # Перша колонка (дати)
            
            fig = go.Figure()

            # Додаємо стовпчик Попереднього місяця (сірий)
            fig.add_trace(go.Bar(
                x=x_axis, y=df.iloc[:, 1],
                name='Лютий',
                marker_color='#A5A5A5', # Екселівський сірий
                text=df.iloc[:, 1], textposition='outside'
            ))

            # Додаємо стовпчик Поточного місяця (зелений)
            fig.add_trace(go.Bar(
                x=x_axis, y=df.iloc[:, 2],
                name='Березень',
                marker_color='#92D050', # Екселівський зелений
            ...
            # Таблиця під графіком (як у скріншоті)
            st.markdown("#### 📋 Таблиця показників")
            st.dataframe(df.T, use_container_width=True) # Транспонуємо для вигляду як у Екселі

        except Exception as e:
            st.info("Для відображення графіка як у Екселі, перевірте структуру вкладки 'Е-Бали'.")
            st.dataframe(df, use_container_width=True, hide_index=True)
    
    else:
        # Стандартний вивід для інших вкладок
        st.dataframe(df, use_container_width=True, hide_index=True)
