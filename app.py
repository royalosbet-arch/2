...
try:
    ...
    if selected_tab == "Мінування":
        ...
        if not df_m.empty:
            ...
            # Перетворюємо суму в float, щоб уникнути помилки JSON serializable
            daily_res = plot_df.groupby('Day_Label')['Кількість'].sum().astype(float).reset_index()
            ...
            overall = df_m.groupby(['Sort_Key', 'Month_Label'])['Кількість'].sum().astype(float).reset_index().sort_values('Sort_Key')
            ...
            with st.expander("📂 ПЕРЕГЛЯНУТИ ТАБЛИЦЮ"):
                ...
                # Перетворюємо все в текст для таблиці, щоб не було помилок серіалізації
                st.dataframe(df_m[display_cols].astype(str), use_container_width=True, hide_index=True)
        else:
        ...
        def bc(d, t, start_col):
            ...
            for u, c in clrs.items():
                ...
                if col_idx:
                    # Перетворюємо в float для надійності
                    v = pd.to_numeric(d.iloc[:, col_idx[0]], errors='coerce').fillna(0).astype(float)
        ...
        # Перетворення значень у float для уникнення помилки
        v1 = pd.to_numeric(raw_df.iloc[1:, 1], errors='coerce').fillna(0).astype(float)
        v2 = pd.to_numeric(raw_df.iloc[1:, 2], errors='coerce').fillna(0).astype(float)
        f = go.Figure()
