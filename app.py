...
POINTS_MAP = {
    ...
    "Мавік": 6, "Мавк": 6,  # исправление опечатки в данных
...
def get_urazh_data(qty, target, status):
    """Нормалізує target перед пошуком у POINTS_MAP."""
    t_clean = str(target).strip()
    # Нормалізація: прибрати зайві пробіли, уніфікувати deashes
    t_norm = t_clean.replace("  ", " ")
    unit_p = POINTS_MAP.get(t_norm, 0)
    if unit_p == 0:
        # Спроба case-insensitive пошуку
...
# =================================================================
# ЄДИНА ФУНКЦІЯ ПАРСИНГУ ДАНИХ (усунення дублювання)
# =================================================================
def parse_battalion_data(conn, unit_names, prefix, cur_m, cur_y):
    """
    Парсить дані з Google Sheets для всіх підрозділів.
    Повертає список словників з уніфікованою структурою.
    """
    all_results = []
    validation_issues = []  # Для збору попереджень про дані
    ...
    for b_name in unit_names:
        ...
        try:
            ...
            for r_idx, r in enumerate(u_rows[1:], start=2):
                ...
                # Перевірка на невідомі типи цілей
                if target != "Мінування":
                ...
                # Обробка колонки мінування (колонка 4, якщо є)
...
def parse_urazh_data(conn, unit_names, prefix, cur_m, cur_y):
    """Парсить дані уражень для порівняльної таблиці."""
    urazh_all_units = []
    for b_name in unit_names:
        sheet_name = f"{prefix}.{b_name}"
        df_unit = None
...
# ⬇️ РОЗШИРЕНЕ МЕНЮ — нові розділи додані
...
try:
    if category == "⚔️ Бригадні звіти":
        # Динамічний список місяців (замість жорстко закодованого)
        available_months = get_dynamic_months(6)
        ...
        # Використання єдиної функції парсингу
    ...
    # =================================================================
    # ðŸ"§ НОВИЙ РОЗДІЛ: ЗАГАЛЬНИЙ ДАШБОРД
    # =================================================================
    elif category == "📊 Дашборд":
        available_months = get_dynamic_months(4)
        ...
        if all_results:
            ...
            # --- Порівняння підрозділів (таблиця) ---
    ...
    # =================================================================
        ...
        if all_results:
            ...
            # --- Стовпчаста діаграма з накопиченням: підрозділ × статус верифікації ---
            ...
            # --- Топ цілей за балами (горизонтальна стовпчаста діаграма) ---
            ...
            # --- Накопичувальний графік балів по днях ---
            ...
            for b in unit_names:
                ...
                # Видаляємо дублікати днів (беремо останнє значення на день)
                unique_days = []
                unique_pts = []
                seen_days = set()
            ...
            # --- Накопичувальна гістограма:aily activity ---
            st.markdown("---")
    ...
    # =================================================================
    # ðŸ"§ НОВИЙ РОЗДІЛ: ВАЛІДАЦІЯ ДАНИХ
    # =================================================================
        ...
        # Перевірка POINTS_MAP на дублікати
        st.markdown("---")
